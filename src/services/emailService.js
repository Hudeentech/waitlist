    ...
    try {
        if (process.env.SMTP_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
            ...
            try {
                ...
                console.warn('SMTP transporter verification failed:', verifyErr && verifyErr.message);
            }
        } else {
            // Fallback to Ethereal for development/testing if SMTP isn't configured
            console.log('SMTP not configured — creating Ethereal test account for development');
            ...
            console.log('Ethereal account created - preview messages at the returned URLs');
        }
    } catch (err) {
...
async function sendMailWithRetry(mailOptions, maxAttempts = 3) {
    ...
    while (attempt < maxAttempts) {
        ...
        try {
            ...
            const transient = ['ETIMEDOUT', 'ESOCKET', 'ECONNRESET', 'EAI_AGAIN', 'ENOTFOUND'].includes(code) || /timeout|connect/i.test(err && err.message);
...
async function sendMailBulk(recipients, subject, html, batchSize = 5, delayMs = 1000) {
    ...
    for (let i = 0; i < recipients.length; i += batchSize) {
        ...
        const promises = batch.map(to => {
            ...
            return sendMailWithRetry(mailOptions, 2)
                .then(result => ({ email: to, ...result }));
        });
...
<body style="background:#f7f7f7; font-family:Arial, sans-serif; padding:0; margin:0;">
    <table align="center" cellspacing="0" cellpadding="0" width="100%" style="max-width:600px; margin:auto;">
        <tr>
            <td style="padding:40px 20px; text-align:center; background:white; border-bottom:3px solid #52AF44; border-radius:10px 10px 0 0;">
                <h1 style="color:#52AF44; font-size:42px; font-weight:bold; margin:0 0 20px 0; letter-spacing:2px;">KORIM</h1>
                <h2 style="color:#52AF44; margin-top:20px;">You're Officially In 🎉</h2>
            </td>
        </tr>

        <tr>
            <td style="background:white; padding:25px 25px 10px 25px; font-size:15px; color:#333;">
                <p>Thank you for joining the KORIM waitlist! ✅</p>

                <p style="margin-top:18px;">You’re now part of the exclusive early community that will get first access before KORIM officially launches in Nigeria.</p>

                <p style="margin-top:18px;">KORIM is the most trusted local service platform in Nigeria — built to connect you instantly with verified artisans, service providers, repair experts, technicians and professionals around you. Fast, safe and stress-free.</p>

                <p style="margin-top:18px;">You will be notified before public launch, see new updates first, and enjoy premium access before anyone else.</p>

                <p style="margin-top:18px;">Stay ready. You’re early — and early always pays.</p>
            </td>
        </tr>

        <tr>
            <td style="background:white; padding:15px; text-align:center;">
                <div style="padding:8px; background:#52AF44; color:white; border-radius:8px; font-size:16px; font-weight:600;">
                    Welcome to KORIM.
                </div>
            </td>
        </tr>
    ...
    const result = await sendMailWithRetry(mailOptions, Number(process.env.EMAIL_MAX_RETRIES) || 3);
    if (!result.success) {
        console.error('Error sending welcome email:', result.error || 'unknown error');
        return false;
    }
    return true;
};
...
const sendLaunchNotification = async (email) => {
    ...
    const result = await sendMailWithRetry(mailOptions, Number(process.env.EMAIL_MAX_RETRIES) || 3);
    if (!result.success) {
        console.error('Error sending launch notification:', result.error || 'unknown error');
        return false;
    }
    return true;
};
